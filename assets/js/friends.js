...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try { await loadLoggedUser(); } catch (error) { console.error(error); }
  ...
  addFriendModal?.addEventListener('show.bs.modal', () => {
    ...
    if (friendSuccess) { friendSuccess.textContent = ''; friendSuccess.style.display = 'none'; }
  ...
  function friendCard(user = {}) {
    return `
      <a href="${profileUrlFor(user)}" class="friend-card community-card friend-card-link-card">
  ...
  function renderFriends() {
    renderLimited(friendsContainer, state.friends.filter(matchesSearch), state.friendsVisible, friendCard, 'Você ainda não tem amigos adicionados.', 'mine');
  }

  function renderExplore() {
    const friendNicknames = new Set(state.friends.map((friend) => userProfileSource(friend).nickname).filter(Boolean));
    const filteredExplore = state.explore.filter((user) => {
      const nickname = userProfileSource(user).nickname;
      return nickname && !friendNicknames.has(nickname);
    }).filter(matchesSearch);

    renderLimited(exploreContainer, filteredExplore, state.exploreVisible, exploreCard, state.query ? 'Nenhum usuário encontrado para a busca.' : 'Digite um nickname para encontrar alunos ou enviar uma solicitação.', 'explore');
  }
  ...
  function renderRequests() {
    ...
    if (!state.received.length) receivedContainer.innerHTML = '<div class="api-empty-state">Nenhuma solicitação recebida.</div>';
    else receivedContainer.innerHTML = state.received.map((req) => requestCard(req.sender, 'received')).join('');

    if (!state.sent.length) sentContainer.innerHTML = '<div class="api-empty-state">Nenhuma solicitação enviada.</div>';
    else sentContainer.innerHTML = state.sent.map((req) => requestCard(req.receiver, 'sent')).join('');
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (profileCard && !interactiveChild) { window.location.href = profileCard.dataset.profileHref; return; }
  ...
  document.addEventListener('keydown', (event) => {
    ...
    event.preventDefault(); window.location.href = profileCard.dataset.profileHref;
  });
  ...
  window.sendFriendRequest = async function(nicknameArg = '') {
    ...
    try {
      ...
      if (!response.ok) { friendError.textContent = getApiError(data, 'Erro ao enviar solicitação.'); friendError.style.display = 'block'; return; }
  ...
  window.removeFriend = async function(nickname) {
